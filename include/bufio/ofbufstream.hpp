...
public:
	...
	basic_ofbufstream(size_t buffer_size, const std::filesystem::path& path) :
...
private:
	...
	inline void adump() {
		m_bufuture = std::async(std::launch::async, [this]() {
			this->m_stream.write(reinterpret_cast<char*>(this->m_buf2.data()),
...
struct __ofbufstream_dispatcher {};

template <class T>
struct __ofbufstream_dispatcher<T, basic_buffer_tag> {
	using type = basic_ofbufstream<T>;
};

template <class T>
struct __ofbufstream_dispatcher<T, double_buffer_tag> {
	using type = async_ofbufstream<T>;
};
